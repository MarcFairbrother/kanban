...
describe('AllLists.vue', () => {
  ...
  it('renders a single li when state includes a single list', () => {
    ...
    expect(wrapper.find('li.lists__item--list').html()).toContain('data-id="1"');
  });
  ...
  it('renders two li elements when state includes two lists', () => {
    ...
    expect(wrapper.find('li.lists__item--list[data-id="1"]').exists()).toBe(true);
    expect(wrapper.find('li.lists__item--list[data-id="2"]').exists()).toBe(true);
  });
  ...
  it('dispatches updateListTitle when title element is edited', async () => {
    const store = new Vuex.Store({
      getters: {
        getLists: () => [{ title: 'List', id: 1 }],
      },
    });
    const handleTitleEdit = jest.fn((e) => updateListTitle({ newTitle: e.input.target.innerText, id: 1 }));
    ...
    expect(updateListTitle).toHaveBeenCalledWith({ newTitle: 'foo', id: 1 });
  });
});
